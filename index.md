...
Includes both postpaid and prepaid subscribers.
...
full dataset above) to a single catch-all ‘downgrade’ class that represents any of
cancelled, switched to prepaid or the original downgraded class.
...
downgrade dataset; i.e., so that only postpaid subscribers remain.
